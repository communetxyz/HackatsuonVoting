...
### Edge Cases
1. **Wallet switches account mid-session**: Detect account change event, refresh all data for new address
2. **Network change during transaction**: Detect network change, show error, prompt to switch back
3. **Contract upgrade/redeployment**: Version detection, show migration notice
4. **Simultaneous votes (race condition)**: Let contract enforce rules, show appropriate error
5. **Long transaction confirmation times**: Show pending state with estimated time, allow backgrounding
6. **Tie in voting results**: Contract uses "first project wins" tiebreaker, display accordingly
7. **Very long project descriptions**: Truncate with "read more", modal for full content
8. **Invalid media URLs**: Show fallback images/placeholders
9. **No Web3 provider on mobile**: Show mobile wallet browser instructions

### Design Concessions
1. **No vote editing**: Blockchain immutability prevents changes; show clear confirmation before voting
2. **Gas costs visible**: Users pay gas fees; show estimated cost before transaction
3. **No automatic updates**: Manual refresh required to see latest data (no polling/WebSocket)
4. **No admin UI**: Project registration and voting resolution handled via Forge scripts
5. **Single-network deployment**: Must manually switch networks for testnet vs mainnet
6. **No vote privacy**: All votes are public and linked to addresses
7. **No fractional votes**: Each vote counts as 1, no weighted voting
8. **Winner determination by votes only**: No manual overrides or tiebreaker criteria beyond contract logic

## 7. Open Questions

1. **Design System**: Should we follow an existing design system or create custom UI components?
2. **Vote Privacy**: Is the public nature of votes (address linked) acceptable to all stakeholders?
3. **Project Media**: Should images/videos be hosted on IPFS or allow arbitrary URLs?
4. **Mobile Experience**: Should there be a dedicated mobile app or PWA, or is mobile web sufficient?
5. **Internationalization**: Do we need multi-language support?
6. **Analytics**: What user behavior tracking is required (privacy-preserving)?
7. **Accessibility**: What WCAG compliance level is required?
8. **Browser Support**: Which browsers and versions must be supported?
9. **Data Refresh**: Should users manually refresh the page, or add a "Refresh" button for latest data?
10. **Voting Schedule**: Should the frontend display voting start/end times (if tracked off-chain)?

## 8. Glossary / References

### Terms
- **Wallet**: Cryptocurrency wallet application (e.g., MetaMask) used for identity and transaction signing
- **Web3 Provider**: JavaScript API for interacting with Ethereum-compatible blockchains
- **Transaction**: Blockchain operation that modifies state (requires gas fees and confirmation)
- **Gas**: Fee paid in native token for executing blockchain transactions
- **RPC**: Remote Procedure Call endpoint for reading blockchain data
- **Contract Owner**: Wallet address that deployed the contract and has admin privileges
- **Voter**: Any wallet address that participates in voting
- **Project ID**: Unique uint256 identifier for each registered project (starts at 1)
- **Voting Resolved**: Final state after admin calls resolveVoting(), winner determined

### Contract Functions Reference (Frontend Use)
- `getVotingData(address)`: Fetches all voting state in single call (primary data source)
- `vote(uint256)`: Casts vote for project (max 2 per address)
- `getMyVotes()`: Returns array of project IDs user voted for
- `getProject(uint256)`: Returns details for specific project
- `projectCount()`: Returns total number of registered projects
- `totalVoters()`: Returns count of unique addresses that voted
- `winnerProjectId()`: Returns winning project ID (0 if not resolved)
- `votingResolved()`: Returns true if voting is finalized

### Admin Functions (Forge Script Only - Not Frontend)
- `registerProject(...)`: Adds new project via script
- `registerProjects(...)`: Batch project registration via script
- `resolveVoting()`: Finalizes voting and determines winner via script

### Links
- Smart Contract Interface: `src/IHackathonVoting.sol`
- Contract Deployment: Gnosis Chain (mainnet), Holesky (testnet)
- Block Explorers: GnosisScan (Gnosis), Etherscan (Holesky)
- Web3 Provider Docs: [EIP-1193](https://eips.ethereum.org/EIPS/eip-1193)
